...
- [ ] Verify with `rg -n "Default: =ThisItem.ScannedQuantity|colSubmitErrors|CountRows\\(colScannedItems\\)" apps/inventory-tracker-powerapps\\RapidScan.pa.yaml`.
...
- [ ] Verify with `rg -n "varShowArchiveConfirm|colArchiveErrors|colArchiveCandidates|Archive selected items" apps/inventory-tracker-powerapps\\Inventory.pa.yaml apps/inventory-tracker-powerapps\\ItemList.pa.yaml`.
...
- [ ] Verify with `rg -n "current session|varDefaultQuantity|Click here, then scan|Scan the next barcode" apps/inventory-tracker-powerapps`.
...
- [ ] Verify with `rg -n "HelpButton_|HelpOverlay_|ctxShowHelp" apps/inventory-tracker-powerapps`.
...
- [ ] Run `git diff -- apps/inventory-tracker-powerapps docs` and review the changed formulas for accidental unrelated edits.
